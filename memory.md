...
- **Supabase generic loss (Next.js 16)**: `createServerClient<Database>` and `createBrowserClient<Database>` from `@supabase/ssr` lose their `Database` generic under Next.js 16 strict TypeScript. All `.from()` calls then infer `never`, causing TS errors on every query result and insert payload. **Fix applied in `lib/supabase/server.ts` and `lib/supabase/client.ts`**: annotate the wrapper function's return type as `SupabaseClient<Database>` and cast the returned client with `as unknown as SupabaseClient<Database>`. This restores correct inference for all callers without per-call casts. API routes that create their own admin client must use `const insertPayload = { ... } as any` as an additional escape for `.insert()` calls (see `api/invite/route.ts`).
