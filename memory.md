...
- **Supabase `.from()` inferring `never` (root cause)**: Using `Views: Record<string, never>` in the `Database` type made every string a valid view name (since `keyof Record<string, never>` = `string`). Supabase's `.from()` has overloads for both Tables and Views — with Views matching every string and returning `never`, TypeScript resolved all `.from()` calls to `never`. **Fix**: change `Views` and `Enums` in `database.ts` from `Record<string, never>` to `{ [_ in never]: never }`. The mapped type has `never` as its only valid key, so no string matches the Views overload, and Tables inference is unambiguous. Always use `{ [_ in never]: never }` for empty Views/Enums/CompositeTypes in Supabase Database types — never `Record<string, never>`.
