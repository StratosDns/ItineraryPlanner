...
- **Cost splits** — equal or custom. Per-split settled toggle (fine-grained). Settlement summary uses a pair-wise net-debt algorithm (N-person): for each ordered pair (A, B) per currency, net A's unsettled gross debt to B against B's unsettled gross debt to A; show "A → B: €X" only when net > €0.01. "Settle" button bulk-marks all relevant splits (both directions) as settled in one Supabase call. Payer's own share is excluded from debt rows — shown as info text on the expense card. Multi-currency: computed independently per currency.
- **No SSR for Leaflet** — `dynamic(() => import(...), { ssr: false })` used to avoid window-not-defined errors.
- **Sticky notes zoom-relative scaling** — notes maintain a consistent geographic footprint regardless of zoom level. They grow as you zoom in, shrink as you zoom out. Scale formula: `Math.pow(2, zoom - 13)` clamped to [0.15, 6], so zoom 13 = base size (130px wide). Combined scale = `noteZoomScale(zoom) * note_scale` where `note_scale` is the user's preferred size multiplier.
- **Zoom sync technique** — `zoomanim` event (fires at the START of Leaflet's zoom animation, before tiles begin animating) is used to update the inner div's CSS `scale()`. The inner div has `transition: transform 0.25s cubic-bezier(0,0,0.25,1)` matching Leaflet's tile animation duration. This makes notes scale in visual lockstep with the map tiles. `zoomend` is a fallback for non-animated zooms.
- **Sticky notes user-resizable** — drag the bottom-right corner handle. Drag suppresses CSS transition for immediate feedback. On mouseup: `onNoteScaleChange` → `setMapNotes` → notes effect rebuilds. Scale stored in `note_scale` on `map_notes` (run8.sql). Resize delta is divided by current `noteZoomScale` so the geographic size (not pixel size) changes consistently regardless of which zoom level the user resizes at.
- **Critical Leaflet pitfall** — never set `el.style.transform` on `marker.getElement()` (the outer marker div). Leaflet sets `transform: translate3d(X,Y,0)` on that element for geographic positioning. Overwriting it snaps the marker to (0,0) on the pane. Always target an inner child element (`[data-note-inner]`).
