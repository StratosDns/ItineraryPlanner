...
- **Sticky notes fixed pixel size** — notes maintain the same screen pixel size regardless of map zoom (standard Leaflet `divIcon` behavior). `note_scale` multiplies the base dimensions (130×52px) to set the note's absolute screen size. No zoom-relative scaling.
- **Sticky notes user-resizable** — drag the bottom-right corner handle to resize. During drag: CSS `scale()` preview on the inner div (outer Leaflet marker element untouched — overwriting its `transform` would clobber Leaflet's `translate3d` positioning). On mouseup: `onNoteScaleChange` → `setMapNotes` → notes effect rebuilds icon at correct pixel dimensions. Scale stored in `note_scale` on `map_notes` (run8.sql).
- **Critical Leaflet pitfall** — never set `el.style.transform` on `marker.getElement()` (the outer marker div). Leaflet sets `transform: translate3d(X,Y,0)` on that element for positioning. Overwriting it snaps the marker to (0,0). Always apply visual transforms to an inner child element.
